...
class MyDestinationListFragment : Fragment() {
    ...
    private fun initializeUI(rootView: View) {
        recyclerView = rootView.findViewById<RecyclerView>(R.id.recyclerView).apply {
            ...
            adapter = DestinationAdapter(requireActivity(), mutableListOf(), true)
        }

        rootView.findViewById<Button>(R.id.add_button).setOnClickListener {
            NavHostFragment.findNavController(this)
                .navigate(R.id.action_myDestinationListFragment_to_addDestinationFragment)
        }
    ...
    private fun setupViewModel() {
        ...
        if (isOnline()) {
            FirebaseAuth.getInstance().currentUser?.email?.let {
                viewModel.fetchMyDestinationsFromFirestore(
                    it
                )
            }
        } else {
